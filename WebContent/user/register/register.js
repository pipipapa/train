(function(window, $, undefined) {
	...
	function checkInviteCode(ele) {
			if (ele.value && (ele.value.length == 11 || ele.value.length == 6)) {
				if ((ele.value.length == 6 && new RegExp("^([a-zA-Z0-9])+$").test(ele.value)) || (ele.value.length == 11 && new RegExp("^((13[0-9])|(15[0-9])|(18[0-9])|14[0-9]|17[0-9])[0-9]{8,8}$").test(ele.value)) || (("c" == ele.value[0] || "C" == ele.value[0]) && new RegExp("^([0-9])+$").test(ele.value.substr(1)))) {
    ...
    */
	...
	function checkData(ele, blurCheck) {
			...
			switch (ele.name) {
				case "tel":
					if (!value ||($('.J_zoneHid').val() == '0086' && new RegExp("^((13[0-9])|(15[0-9])|(18[0-9])|14[0-9]|17[0-9])[0-9]{8,8}$").test(value) == false) 
							|| ($('.J_zoneHid').val() != '0086' && new RegExp("^[0-9]{5,13}$").test(value) == false) ) {
						$("#info_submit").attr("disabled", "disabled").addClass("login_btn_gray");
						$("#tel-tip .input-tip-inner span").removeClass('reg_mes mes_phone').html("请输入正确的邮箱");
						$("#tel-tip").addClass("err");
						return false;
					} else if (!telCodeFlag) {
						$("#info_submit").attr("disabled", "disabled").addClass("login_btn_gray");
						var url = "/isEmailAvailable";
	...
	$("#info_submit").bind("click", function() {
		...
		if (!telCodeFlag || !flagIdentify || !flagCode || ($.trim($("#invite_code").val()) != "" && !invite_codeFlag)) {
			return false;
		}
		var data = {
			intlCode: $('.J_zoneHid').val(),
